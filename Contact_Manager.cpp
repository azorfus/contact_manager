...
void edit_contact()
{
	...
	else
	{
		...
		std::cout << "Line to edit: ";
		std::cin >> lineED;
		std::cin.ignore();
	
		if(lineED < 1, lineED - 1 > lines.size())
		{
			std::cout << "Line specified does not exist!\n";
			returnM();
		}
		...
		{
			std::string name, phone, email;
			lines.erase(lines.begin() + lineED - 1);
			...
			std::string newcontact = name + '|' + phone + '|' + email;

			lines.insert(lines.begin() + lineED - 1, newcontact);
			std::cout << "Contact edit done!\n";
